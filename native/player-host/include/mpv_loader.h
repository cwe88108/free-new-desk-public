...
class MpvLoader {
 ...
 private:
  ...
  mutable std::mutex loadMutex_;std::thread eventThread_;std::atomic<bool> stopEvents_{false};std::uint64_t loadGeneration_=0;bool loadStarted_=false;std::string loadId_;std::string loadStatus_="idle";std::string loadError_;std::string headerFields_;std::int64_t activeEntryId_=-1,eventEntryId_=-1;
