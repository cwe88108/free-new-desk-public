...
const assetsDir=path.resolve('assets'),pngPath=path.join(assetsDir,'app-icon.png'),icoPath=path.join(assetsDir,'app-icon.ico'),rendererPngPath=path.resolve('apps/desktop/src/renderer/assets/app-icon.png');
...
await writeFile(rendererPngPath,source);
const temp=await mkdtemp(path.join(os.tmpdir(),'fnd-icon-'));
try{
  const ps=`Add-Type -AssemblyName System.Drawing;$src=[Drawing.Image]::FromFile($env:FND_ICON_SRC);${sizes.map(s=>`$b=New-Object Drawing.Bitmap(${s},${s},[Drawing.Imaging.PixelFormat]::Format32bppArgb);$g=[Drawing.Graphics]::FromImage($b);$g.InterpolationMode=[Drawing.Drawing2D.InterpolationMode]::HighQualityBicubic;$g.SmoothingMode=[Drawing.Drawing2D.SmoothingMode]::HighQuality;$g.DrawImage($src,0,0,${s},${s});$g.Dispose();$b.Save((Join-Path $env:FND_ICON_OUT '${s}.png'),[Drawing.Imaging.ImageFormat]::Png);$b.Dispose();`).join('')}$src.Dispose();`;
  ...
  console.log(`Validated requested Free New Desk artwork ${dim.width}x${dim.height}; synchronized the renderer PNG and materialized alpha-preserving multi-size ICO (${sizes.join(',')}) without redrawing the supplied icon.`);
