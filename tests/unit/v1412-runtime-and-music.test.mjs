...
test('v1.4.12 drpy2 worker executes ESM and synchronous broker req without blocking parent',async()=>{
 ...
 try{await ask({type:'init',sourceId:'test',entryUrl:entry,modules:{[entry]:code},ruleExt:'fixture-rule',bridgeDir:dir,profileId:'fixture-drpy2',hostAbiVersion:'fixture-host/3',instantiation:'object',initContractId:'rule-ext-string-v1',requiredCapabilities:['req-sync','pdfh','pdfa','pd'],methodNames:['home','search']});const home=await ask({type:'call',method:'home',args:[]});assert.equal(home.class[0].type_name,'Demo');const result=await ask({type:'call',method:'search',args:['needle']});assert.match(result.list[0].vod_name,/needle OK/);}finally{child.kill();await rm(dir,{recursive:true,force:true});}
});
test('v1.4.14 music progress normalizes Windows paths before URL parsing and gates native samples by session identity',async()=>{
...
test('v1.4.14 lyrics prefer local synced sidecars before online candidates',async()=>{
 const service=await text('apps/desktop/src/main/music-service.ts'),online=await text('apps/desktop/src/main/music-online.ts');const fn=service.slice(service.indexOf('async function readLyricsInfo'),service.indexOf('async function addNetworkSource'));assert.ok(fn.indexOf('track.lyricsPath')<fn.indexOf('findOnlineLyrics'));assert.match(fn,/online\?\.kind==='synced'/);assert.match(fn,/parseLrc\(local\)\.length/);assert.match(online,/const title=sanitizeMusicMetadata\(track\.title\),artist=sanitizeMusicMetadata\(track\.artist\)/);assert.match(online,/new URLSearchParams\(\{track_name:title,artist_name:artist\}\)/);assert.doesNotMatch(online,/track_name:title,artist_name:artist,album_name/);
});
...
test('v1.4.15 packages drpy2 worker and exposes the new version',async()=>{
 const root=JSON.parse(await text('package.json'));assert.equal(root.version,'1.4.17');assert.ok(root.build.files.includes('services/drpy2-worker/dist/**/*'));assert.match(root.scripts['build:packages'],/@free-new-desk\/drpy2-worker/);
});

test('v1.4.16 uses the supplied transparent icon across renderer window tray and installer wiring',async()=>{
  ...
  assert.match(iconScript,/alpha-preserving multi-size ICO/);
...
test('v1.4.14 generic ESM engines are not misclassified as drpy2',()=>{
 const generic=`import helper from './helper.js'; export default {init(){},home(){},category(){},detail(){},search(){},play(){}}`;
 const result=classifyRuntimeFlavor({endpoint:'https://example.com/lf_search3_min.js',ext:''},generic);
 assert.equal(result.flavor,'unsupported');assert.ok(result.signals.includes('source:export-default'));assert.ok(result.signals.includes('source:esm-import'));
 const explicit=classifyRuntimeFlavor({endpoint:'https://example.com/drpy2.min.js',ext:''},generic);assert.equal(explicit.flavor,'drpy2-esm');
});

test('v1.4.14 drpy1 ESM remains outside the drpy2 profile',()=>{
 const drpy1=`import helper from './helper.js'; const VERSION='drpy1 3.9.49beta36'; function DRPY(){}; export default {init(){},home(){},category(){},detail(){},search(){},play(){},DRPY}`;
 const result=classifyRuntimeFlavor({endpoint:'https://example.com/drpy.min.js',ext:'https://example.com/rule.js'},drpy1);
 assert.equal(result.flavor,'unsupported');assert.ok(result.signals.includes('source:drpy-core'));assert.ok(!result.signals.includes('source:drpy2'));
});
