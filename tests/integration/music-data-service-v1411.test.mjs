...
test('music migrations persist sources tracks and favorites through v16',()=>{
  const dir=mkdtempSync(path.join(os.tmpdir(),'free-new-desk-music-'));
  try{
    const db=new DataService(path.join(dir,'app.db'));assert.equal(db.getDatabaseVersion(),16);
