...
export function mergeHeaders(...sets:Array<Record<string,string>|undefined>):Record<string,string>{const result=new Headers();for(const set of sets??[])for(const[key,value]of Object.entries(set??{}))result.set(key,value);return Object.fromEntries(result.entries());}
class SourceCookieStore{readonly #jars=new Map<string,Map<string,string>>();#key(sourceId:string,url:string):string{return `${sourceId}\u0000${new URL(url).hostname.toLowerCase()}`;}header(sourceId:string,url:string):string|undefined{const jar=this.#jars.get(this.#key(sourceId,url));if(!jar||jar.size===0)return undefined;return[...jar].map(([name,value])=>`${name}=${value}`).join('; ');}capture(sourceId:string,url:string,headers:Headers):void{const raw=(headers as Headers&{getSetCookie?:()=>string[]}).getSetCookie?.()??(headers.get('set-cookie')?[headers.get('set-cookie') as string]:[]);if(raw.length===0)return;const key=this.#key(sourceId,url);const jar=this.#jars.get(key)??new Map<string,string>();for(const item of raw){const pair=item.split(';',1)[0]??'';const split=pair.indexOf('=');if(split<=0)continue;const name=pair.slice(0,split).trim();const value=pair.slice(split+1).trim();if(/max-age\s*=\s*0/i.test(item)||value==='')jar.delete(name);else jar.set(name,value);}this.#jars.set(key,jar);}clear(sourceId:string):void{for(const key of[...this.#jars.keys()])if(key.startsWith(`${sourceId}\u0000`))this.#jars.delete(key);}}
function linkedSignal(timeoutMs:number,external?:AbortSignal):{signal:AbortSignal;dispose:()=>void}{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(new DOMException('Request timeout','TimeoutError')),timeoutMs);const abort=()=>controller.abort(external?.reason);if(external)external.addEventListener('abort',abort,{once:true});return{signal:controller.signal,dispose:()=>{clearTimeout(timer);external?.removeEventListener('abort',abort);}};}
function retryable(error:unknown):boolean{return error instanceof TypeError||(error instanceof DOMException&&/Timeout|Abort/i.test(error.name));}
