...
export type PlaybackDomain='vod'|'live'|'music';
export type PlaybackSessionStatus='idle'|'requested'|'loading'|'playing'|'paused'|'stopped'|'failed'|'ended'|'superseded';
export type PlaybackInitiator='user'|'auto-next'|'auto-route'|'retry'|'history'|'restore';
export interface PlaybackSessionSnapshot{
  generation:number;sessionId:string;requestId:string;loadId?:string;domain:PlaybackDomain|null;status:PlaybackSessionStatus;initiator?:PlaybackInitiator;
  sourceId?:string;channelId?:string;routeIndex?:number;routeCount?:number;videoId?:string;episodeId?:string;trackId?:string;queueItemId?:string;
  startedAt?:number;updatedAt:number;endReason?:'eof'|'stop'|'replaced'|'failed'|'unknown';error?:string;
}
...
export const MusicNetworkSourceRequestSchema=z.object({kind:z.enum(['webdav','smb']),name:z.string().min(1).max(120),root:z.string().min(1).max(4096),username:z.string().max(512).optional(),password:z.string().max(2048).optional(),rememberCredential:z.boolean().optional(),disconnectConflicts:z.boolean().optional()});
...
export interface PlayerObservation{domain?:PlaybackDomain|null;requestId?:string;sessionGeneration?:number;sessionStatus?:PlaybackSessionStatus;sessionLoadId?:string;}
...
export const PlayerCommandSchema=z.discriminatedUnion('command',[
  z.object({command:z.literal('pause'),value:z.boolean()}),z.object({command:z.literal('seek'),value:z.number(),absolute:z.boolean().optional()}),z.object({command:z.literal('volume'),value:z.number().min(0).max(100)}),z.object({command:z.literal('mute'),value:z.boolean()}),z.object({command:z.literal('speed'),value:z.number().min(0.25).max(4)}),z.object({command:z.literal('stop')}),z.object({command:z.literal('subtitle-add'),value:z.string().min(1)}),z.object({command:z.literal('audio-track'),value:z.string()}),z.object({command:z.literal('subtitle-track'),value:z.string()}),z.object({command:z.literal('subtitle-delay'),value:z.number()}),z.object({command:z.literal('audio-delay'),value:z.number()}),z.object({command:z.literal('aspect-ratio'),value:z.string().min(1).max(32)}),z.object({command:z.literal('hwdec'),value:z.enum(['auto-safe','no'])}),z.object({command:z.literal('pip'),value:z.boolean()}),z.object({command:z.literal('fullscreen'),value:z.boolean()}),z.object({command:z.literal('screenshot'),value:z.string().min(1)}),z.object({command:z.literal('osd-text'),value:z.string().min(1).max(2000),durationMs:z.number().int().min(250).max(10000).optional()}),z.object({command:z.literal('window-sync'),x:z.number(),y:z.number(),width:z.number().int().min(1),height:z.number().int().min(1),scale:z.number().positive().max(8).optional(),visible:z.boolean()})
]);
...
export const PlaybackEpisodeRequestSchema=z.object({sourceId:z.string().min(1),videoId:z.string().min(1),videoName:z.string().min(1),flag:z.string(),episodeId:z.string().min(1),episodeName:z.string().min(1),fromEpisodeId:z.string().min(1).optional(),poster:z.string().max(4096).optional(),episodeListJson:z.string().max(262144).optional(),initiator:z.enum(['user','auto-next','history','restore']).optional()});
